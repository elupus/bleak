...
from bleak.backends.bluezdbus import descriptor
...
MOCK_DESCRIPTOR_1_PATH = "{}/desc{:04}".format(MOCK_CHARACTERISTICS_1_PATH, MOCK_DESCRIPTOR_1_HANDLE)
...
def _interfaces_removed(path, interface):
    return device_message(
        "InterfacesRemoved", [path, [interface]]
    )
...
def test_remove_service():
    ...
    characteristics_1 = client.services.characteristics.get(MOCK_CHARACTERISTICS_1_HANDLE)
    ...
    client._parse_msg(_interfaces_removed(MOCK_DESCRIPTOR_1_PATH, GATT_DESCRIPTOR_INTERFACE))
    ...
    client._parse_msg(_interfaces_removed(MOCK_CHARACTERISTICS_1_PATH, GATT_CHARACTERISTIC_INTERFACE))
